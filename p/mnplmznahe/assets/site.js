...
const L = LANG === 'en' ? {
  demo: 'This is a concept page: nothing was sent. On the live site your request goes straight to the business by email.',
  sending: 'Sending…', sent: '✅ Request sent. We will get back to you soon.', fail: 'Could not send. Please call us — the number is at the top of the page.',
  offline: 'No connection. Check the internet and try again, or call us.', of: 'of', map: 'Map',
} : {
  demo: 'Это демо: заявка никуда не ушла. На настоящем сайте она сохраняется на хостинге, приходит на почту и в Telegram.',
  sending: 'Отправляем…', sent: '✅ Заявка отправлена. Скоро свяжемся с вами.', fail: 'Не получилось отправить. Позвоните нам — номер вверху страницы.',
  offline: 'Нет связи. Проверьте интернет и попробуйте ещё раз или позвоните нам.', of: 'из', map: 'Карта',
};
...
for (const form of $$('form.lead-form')) {
  ...
  // служебные поля есть только у своего обработчика (send.php); у внешнего сервиса форм (lang en + formAction) их нет
  const set = (name, v) => { if (form.elements[name]) form.elements[name].value = v; };
  set('js', '1');
  set('page', location.pathname);
  const q = new URLSearchParams(location.search);
  set('source', q.get('utm_source') || q.get('src') || (document.referrer && !document.referrer.startsWith(location.origin) ? new URL(document.referrer).hostname : ''));
  ...
  form.addEventListener('submit', async (ev) => {
    ev.preventDefault();
    set('elapsed', String(Math.round(performance.now() - loaded)));
    const data = Object.fromEntries(new FormData(form));
    const check = validateLead(FIELD_SPEC(form), data, { lang: LANG, consent: Boolean(form.elements.consent) });
    ...
    if ('demo' in form.dataset) {
      ...
      status.textContent = form.dataset.demoText || L.demo;
      return;
    }
    btn.disabled = true;
    ...
    try {
      ...
      else status.textContent = body.error || L.fail;
    } catch {
      status.textContent = L.offline;
...
for (const sec of $$('.gallery')) {
  ...
  let i = 0, x0 = null;
  const show = (n) => { i = (n + list.length) % list.length; im.src = list[i].src; im.alt = list[i].alt; cap.textContent = `${list[i].cap || list[i].alt} · ${i + 1} ${L.of} ${list.length}`; };
...
for (const box of $$('.map[data-src]')) {
  $('button', box).addEventListener('click', () => {
    const f = Object.assign(document.createElement('iframe'), { src: box.dataset.src, title: L.map, loading: 'lazy', allowFullscreen: true });
...
// ── до/после: ползунок двигает границу; без JS видно «после» и половину «до» ──
for (const ba of $$('.ba')) {
  const range = $('.ba-range', ba);
  range.addEventListener('input', () => ba.style.setProperty('--pos', range.value + '%'));
}

// ── видеоотзывы: плеер RuTube/VK грузится только по нажатию ──────────
for (const v of $$('.video[data-embed]')) {
  $('.video-play', v).addEventListener('click', () => {
    const f = document.createElement('iframe');
    f.src = v.dataset.embed;
    f.title = $('.video-play', v).getAttribute('aria-label');
    f.allow = 'autoplay; encrypted-media; fullscreen; picture-in-picture';
    f.allowFullscreen = true;
    $('.video-play', v).replaceWith(f);
  }, { once: true });
}
